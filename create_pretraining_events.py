...
def create_training_instances(input_files, tokenizer, max_seq_length,
                              ...
                              max_predictions_per_seq, rng, keep_label, label_files):
  ...
  if keep_label:
      for label_file in label_files:
          print("creating instance from {}".format(label_file))
          with open(label_file, "r") as reader:
              while True:
                  ...
                  tokens = line.split(", ")
                  if tokens:
                      all_labels[-1].append(tokens)
  # Remove empty documents
  all_documents = [x for x in all_documents if x]
  if keep_label:
    all_labels = [x for x in all_labels if x]
    temp = list(zip(all_documents, all_labels))
    rng.shuffle(temp)
    all_documents, all_labels = zip(*temp)
  else:
  ...
  if keep_label:
    for _ in range(dupe_factor):
        for document_index in range(len(all_documents)):
          instances.extend(
              create_instances_from_document_label(
                  all_documents, document_index, max_seq_length, short_seq_prob, rng, all_labels))
  else:
    for _ in range(dupe_factor):
        for document_index in range(len(all_documents)):
            instances.extend(
...
def create_instances_from_document_label(
    all_documents, document_index, max_seq_length, short_seq_prob, rng, all_labels):
  """Creates `TrainingInstance`s for a single document."""
  document = all_documents[document_index]
  label = all_labels[document_index]
  # Account for [SEP], [CLS]
...
def main():
    ...
    parser.add_argument("--label_file",
                        ...
                        type=str,
                        required=False,
                        help="The label for test corpus. can be directory with .txt files or a path to a single file")
    ...
    if args.keep_label:
        if os.path.isfile(args.label_file):
            label_files.append(args.label_file)
        elif os.path.isdir(args.label_file):
            label_files = [os.path.join(args.label_file, f) for f in os.listdir(args.label_file) if
                           (os.path.isfile(os.path.join(args.label_file, f)) and f.endswith('.txt'))]
        else:
            raise ValueError("{} is not a valid path".format(args.input_file))
    ...
    instances = create_training_instances(
        ...
        rng, args.keep_label, label_files)
