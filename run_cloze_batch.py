...
def main():
    ...
    for file in list(glob.glob('results/'+args.dataset+'/*.pt')):
        inverse_cloze(init_checkpoint=file, output_dir='results/'+args.dataset, data_dir="data/inverse_cloze/cloze_dataset_NYT.json")
        inverse_cloze(init_checkpoint=file, output_dir='results/'+args.dataset, data_dir="data/inverse_cloze/cloze_dataset_weber_test.json")
