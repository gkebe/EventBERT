...
def wiki70k_preprocess(filename, tuple_to_sen = False, keep_label = False, add_tup=False):
  ...
  if add_tup:
    ...
    paragraphs_lst_words = [list(chain.from_iterable([[i[:-1][j * 30:(j + 1) * 30] for j in range((len(i[:-1]) + 30 - 1) // 30)] for i in k])) for k in paragraphs_lst_words]
    print([i for i in paragraphs_lst_words[0] if len(i)==30])
    paragraph_lst_sentences = ''.join(
          [''.join([' '.join([' '.join(v[i:i + 5][:4]) + tup(i,len(v)-5) for i in range(0, len(v), 5)]) + "\n" for v in j]) for j in
